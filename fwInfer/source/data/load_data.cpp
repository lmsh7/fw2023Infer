...
namespace fw_infer {
    ...
    CSVDataLoader::LoadDataWithHeader(const std::string &file_path, std::vector<std::string> &headers,
                                      char split_char) {
        ...
        CHECK(rows >= 1);
        std::shared_ptr<Tensor<float>>
                input_tensor = std::make_shared<Tensor<float>>
                (1, rows - 1, cols);
        ...
        while (in.good()) {
            ...
            while (line_stream.good()) {
                ...
                try {
                    ...
                    if (row == 0) {
                        if (token.back() == '\r') {
                            token.erase(token.end() - 1); // 删除最后一个字符
                        }
