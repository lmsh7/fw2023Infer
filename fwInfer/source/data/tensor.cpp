...
namespace fw_infer {
    ...
    std::vector<uint32_t> Tensor<float>::shapes() const {
        CHECK(!this->data_.empty());
        return {static_cast<unsigned int>(this->data_.n_slices),
                static_cast<unsigned int>(this->data_.n_rows),
                static_cast<unsigned int>(this->data_.n_cols)};
    }
    ...
    arma::fmat &Tensor<float>::at(uint32_t channel) {
        CHECK_LT(channel, this->data_.n_slices);
    ...
    float Tensor<float>::at(uint32_t channel, uint32_t row, uint32_t col) const {
        CHECK_LT(channel, this->data_.n_slices);
        CHECK_LT(row, this->data_.n_rows);
        CHECK_LT(col, this->data_.n_cols);
    ...
    void Tensor<float>::Padding(const std::vector<uint32_t> &pads, float padding_value) {
        ...
        this->data_.insert_rows(0, pad_top);
        this->data_.insert_cols(0, pad_left);
        this->data_.insert_rows(this->data_.n_rows, pad_bottom);
        this->data_.insert_cols(this->data_.n_cols, pad_right);
    }
    ...
    void Tensor<float>::Fill(const std::vector<float> &values) {
        ...
        for (uint32_t i = 0; i < channels; ++i) {
            ...
            const arma::fmat &channel_data_t = arma::fmat(values.data() + i * planes, rows, cols);
        ...
        for (uint32_t c = 0; c < channel; ++c) {
            ...
            for (uint32_t r = 0; r < rows; ++r) {
                for (uint32_t c_ = 0; c_ < cols; ++c_) {
                    linear_cube.at(cnt++, 0, 0) = matrix.at(r, c_);
                }
            }
        }
        CHECK_EQ(cnt, size);
